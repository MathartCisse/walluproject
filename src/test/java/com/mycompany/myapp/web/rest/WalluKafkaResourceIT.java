...
class WalluKafkaResourceIT {
    ...
    void setup() {
        ...
        WalluKafkaResource kafkaResource = new WalluKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/wallu-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/wallu-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
